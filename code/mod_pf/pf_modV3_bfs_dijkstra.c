...
#pragma omp parallel for schedule(SCHEDULING_METHOD)
  for (int i = 0; i < g->N; i++)
  {
    ...
    if (data->phase_discovered == p)
    {
      ...
      // *_*_*_*_*_*_* Start up code for prefetching *_*_*_*_*_*_*
      // ---------------------------------------------------------
      for (int j = 0; j < STRIDE_2 && j < pf_cur->degree; j++)
      {
        // LVL 1 access[i]
    ...
    if (i + STRIDE + 1 < g->N)
    {
      ...
      pf_cur = elem_at(&g_pf->vertices, i + STRIDE);
      ...
      // LVL 3 access[i + STRIDE] ( - LVL 3 ) [ Always a miss but will be of use in future iterations]
      if (pf_data->phase_discovered == p)
      {
        // LVL 2 needed[i + STRIDE] ( + LVL 2 ) { PREFETCH FOR CURR ITERATION}
        for (int i = 0; i < 19; i++) // 20*64 = 1280 (L1 = )
        {
          _mm_prefetch(pf_data + 64 + 64 * i, _MM_HINT_T0);
          // should figure out if this loop prefetching will be good or evict all previous prefetches
        }
      }
    }
    // #########################################################
